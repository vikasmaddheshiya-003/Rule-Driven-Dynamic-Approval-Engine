...
public class EmployeePendingTransactionController extends SelectorComposer<Window> {
    ...
    public void approveCardRequest() throws Exception {
        ...
        empDao.approveCardRequests(req);

        loadUserTransaction();
        Clients.showNotification("Transaction approved successfully!");
    }
...
public void rejectRequest() throws SQLException
{
  ...
  Request request=requestList.getSelectedItem().getValue();
  empDao.rejectTraxRequest(request);
  loadUserTransaction();
  Clients.showNotification("Transaction Rejected !");
}
